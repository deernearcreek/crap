...
from networks_gan import VoxelMorph, UNet
...
def train_epoch(args, model, train_dataloader, opt, device):
    ...
    for batch in tqdm(train_dataloader):
        ...
        with torch.no_grad():
            ct_synth_mr = G_MR(mr)
            ct_synth_cbct = G_CBCT(cbct)

        opt.zero_grad()
        ct_reg, flow = model(ct_synth_mr, ct_synth_cbct)

        loss_sim = losses.NCC(device='cuda').loss(ct_reg, ct_synth_cbct)
...
def valid_epoch(model, G_MR, G_CBCT, valid_dataloader, device):
    sim_loss = 0
    seg_loss = 0
    smooth_loss = 0
    num_samples = 0
...
def get_config(train_path, test_path, exp_name, model_mr_ct, model_cbct_ct, save_path='./checkpoint/', pretrained=None,
               lr=1e-4, lambda_smooth=20, num_epoch=100, start_epoch=0, batch_size=1, dataset='RadOnc', id=2):
    args = dict()
    args['train_path'] = train_path
    args['test_path'] = test_path
    args['exp_name'] = exp_name
    args['model_mr_ct'] = model_mr_ct
    args['model_cbct_ct'] = model_cbct_ct
...
if __name__ == '__main__':
    ...
    parser.add_argument("model_mr_ct", type=str, default=None, dest='model_mr_ct', help='dir to pretrain mr to '
                                                                                        'ct synthesis model')
    parser.add_argument("model_cbct_ct", type=str, default=None, dest='model_cbct_ct', help='dir to pretrain cbct to '
                                                                                        'ct synthesis model')
    ...
    G_MR = UNet(cdim=1, channels=(16, 32, 64, 128, 128), res=True, gumbel=False).to(device)
    G_CBCT = UNet(cdim=1, channels=(16, 32, 64, 128, 128), res=True, gumbel=False).to(device)
    ...
    if args['pretrained'] is not None:
        if os.path.exists(args['pretrained']):
            ...
            del checkpoint
    checkpoint = torch.load(args['model_mr_ct'])
    G_MR.load_state_dict(checkpoint['G_state_dict'])
    checkpoint = torch.load(args['model_cbct_ct'])
    G_CBCT.load_state_dict(checkpoint['G_state_dict'])
    del checkpoint
    ...
    for epoch in range(args['start_epoch'], args['start_epoch'] + args['num_epoch']):
        total_loss, sim_loss, smooth_loss = train_epoch(args, model, G_MR, G_CBCT, train_dataloader, optimizer, device)
        ...
        valid_sim_loss, valid_seg_loss, valid_smooth_loss = valid_epoch(model, G_MR, G_CBCT, valid_dataset, device)
