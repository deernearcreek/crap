...
def train_reg_epoch(args, G, train_dataloader, opt_G, device, scaler=None):
    checkpoint = torch.load('JSRCascadeonnewdata0_l1100_multi0.3_reg100_seg6_ep99.pt',map_location=device)
    for name, module in G.named_parameters():
        ...
        # elif 'decoder_reg.'+name[17:] in checkpoint['G_state_dict'].keys():
        #     module.data.copy_(checkpoint['G_state_dict']['decoder_reg.'+name[17:]])
        #     if 'decoder_reg.'+name[17:] != 'decoder_reg.flow4.weight' and 'decoder_reg.'+name[17:] != 'decoder_reg.flow4.bias':
                # module.requires_grad = False
    ...
    for _, batch in tqdm(enumerate(train_dataloader), 'reg phase:'):
        ...
        if scaler is not None:
            with torch.cuda.amp.autocast():
                ...
                var = flows[4]
                synths_warped = [G.decoder_reg.STN4_1(ct_moving_synths[0].detach(), flows[0]),
                ...
                if args['lambda_multi'] > 0:
                    mrs_warped = [G.decoder_reg.STN4_1(mr_moving, flows[0]),
                ...
                if args['weak_supervision']:
                    ...
                    segs_warped = [G.decoder_reg.STN4_1(seg_moving, flows[0]),
                ...
                loss_unc = 0.5*torch.log(var)+(flows[0]+gt)**2/2*var
...
def save_model(G, D_MR, D_CBCT, opt_G, opt_D_MR, opt_D_CBCT, args, epoch):
    torch.save({
        ...
        'args': args
    }, os.path.join(args['save_path'], 'JSRENS_layer_0.5_{}_ep{}.pt'.format(args['exp_name'], epoch)))
...
if __name__ == '__main__':
    ...
    G = JSRCascade(cdim=1, channels=(16, 32, 64, 128, 128), image_size=(128, 160, 128),
                   separate_decoders=args['separate_decoders'], res=True, version='v6').to(device)
