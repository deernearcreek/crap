...
def train_reg_epoch(args, G, train_dataloader, opt_G, device, scaler=None):
    ...
    for _, batch in tqdm(enumerate(train_dataloader), 'reg phase:'):
        ...
        if scaler is not None:
            ...
            scaler.step(opt_G)
            scaler.update()
            print(G.decoder_reg.flow4.weight.grad.mean().item())
        ...
        print('MIN values')
        print('alphax:%.4f'%alpha_x.min(),'alphay:%.4f'%alpha_y.min(),'alphaz:%.4f'%alpha_z.min(),'betax:%.4f'%beta_x.min(),\
            'betay:%.4f'%beta_y.min(),'betaz:%.4f'%beta_z.min(),'nu_x:%.4f'%mu_x.min(),'nu_y:%.4f'%mu_y.min(),\
                'nuz:%.4f'%mu_z.min(),'omegax:%.4f'%omega_x.min(),'omegay:%.4f'%omega_y.min(),'omegaz:%.4f'%omega_z.min())
        print('MAX values')
        print('alphax:%.4f'%alpha_x.max(),'alphay:%.4f'%alpha_y.max(),'alphaz:%.4f'%alpha_z.max(),'betax:%.4f'%beta_x.max(),\
            'betay:%.4f'%beta_y.max(),'betaz:%.4f'%beta_z.max(),'nu_x:%.4f'%mu_x.max(),'nu_y:%.4f'%mu_y.max(),\
                'nuz:%.4f'%mu_z.max(),'omegax:%.4f'%omega_x.max(),'omegay:%.4f'%omega_y.max(),'omegaz:%.4f'%omega_z.max())
    ...
    for _, batch in tqdm(enumerate(train_dataloader), 'joint phase:'):
        ...
        if scaler is not None:
            with torch.cuda.amp.autocast():
                ...
                mu_z = uncert[0,8,:,:,:]
                omega_z = 2*beta_z*(1+mu_z)
                loss_uncx = 0.5*torch.log(math.pi/mu_x)-alpha_x*torch.log(omega_x)+(alpha_x+0.5)*torch.log((flows[0][0,0,:,:,:]+gtx)**2*mu_x+omega_x)+torch.lgamma(alpha_x)-torch.lgamma(alpha_x+0.5)\
                            +lunc*(2*mu_x+alpha_x)*torch.abs((flows[0][0,0,:,:,:]+gtx))
                loss_uncy = 0.5*torch.log(math.pi/mu_y)-alpha_y*torch.log(omega_y)+(alpha_y+0.5)*torch.log((flows[0][0,1,:,:,:]+gty)**2*mu_y+omega_y)+torch.lgamma(alpha_y)-torch.lgamma(alpha_y+0.5)\
                            +lunc*(2*mu_y+alpha_y)*torch.abs((flows[0][0,1,:,:,:]+gty))
                loss_uncz = 0.5*torch.log(math.pi/mu_z)-alpha_z*torch.log(omega_z)+(alpha_z+0.5)*torch.log((flows[0][0,2,:,:,:]+gtz)**2*mu_z+omega_z)+torch.lgamma(alpha_z)-torch.lgamma(alpha_z+0.5)\
                            +lunc*(2*mu_z+alpha_z)*torch.abs((flows[0][0,2,:,:,:]+gtz))
                synths_warped = [G.decoder_reg.STN4(ct_moving_synths[0].detach(), flows[0]),
                                 G.decoder_reg.STN3(ct_moving_synths[1].detach(), flows[1]),
                                 G.decoder_reg.STN2(ct_moving_synths[2].detach(), flows[2]),
                                 G.decoder_reg.STN1(ct_moving_synths[3].detach(), flows[3])]
                ...
                loss_reg = (loss_sim + loss_smooth + loss_seg) / 4
                loss_unc = loss_uncx+loss_uncy+loss_uncz
...
def save_model(G, D_MR, D_CBCT, opt_G, opt_D_MR, opt_D_CBCT, args, epoch):
    torch.save({
        ...
        'args': args
    }, os.path.join(args['save_path'], 'JSREDL_max_lastlayer_rnewflow_sup_{}_ep{}.pt'.format(args['exp_name'], epoch)))
...
if __name__ == '__main__':
    ...
    # Prepare Tensorboard
    boardio = SummaryWriter(log_dir='./log/JSRCascade_max_{}_{}_{}_{}_{}_{}'.format(args['exp_name'], args['lambda_l1'],
    ...
    for i in range(args['schedule'][1]):
        ...
        log3_yloss,log1_zloss,log2_zloss,log3_zloss,reg_xloss,reg_yloss,reg_zloss= train_reg_epoch(args, G, train_dataloader, opt_G, device, scaler)
