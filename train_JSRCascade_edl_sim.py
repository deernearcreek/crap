    ...
    for _, batch in tqdm(enumerate(train_dataloader), 'joint phase:'):
        ...
        if scaler is not None:
            with torch.cuda.amp.autocast():
                ...
                loss_unc = (0.5*torch.log(math.pi/mu_x)-alpha_x*torch.log(omega_x)+(alpha_x+0.5)*torch.log((1-sim).detach()**2*mu_x+omega_x)+torch.lgamma(alpha_x)-torch.lgamma(alpha_x+0.5)\
                                +args['lambda_inunc']*(2*mu_x+alpha_x)*abs((1-sim).detach())).mean()
...
def save_model(G, D_MR, D_CBCT, opt_G, opt_D_MR, opt_D_CBCT, args, epoch):
    torch.save({
        ...
        'args': args
    }, os.path.join(args['save_path'], 'JSREDLuns_all_sim_md4_0.1n2_{}_ep{}.pt'.format(args['exp_name'], epoch)))
...
def get_config(train_path, test_path, exp_name, save_path='./checkpoint/', pretrained=None, amp=False,
               ...
               synthesis_checkpoint_mod=10, registration_checkpoint_mod=10, jsr_checkpoint_mod=10, lambda_inunc = 2, lambda_unc = 0.1):
    args = dict()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--lambda_inunc", type=float,
                        dest="lambda_inunc", default=2, help="Lambda inside uncertainty loss function")
    parser.add_argument("--lambda_unc", type=float,
                        dest="lambda_unc", default=0.1, help="uncertainty loss weight")
    ...
    # Prepare Tensorboard
    boardio = SummaryWriter(log_dir='./log/JSRCascade_mysim_{}_{}_{}_{}'.format(args['exp_name'], args['lambda_l1'],
