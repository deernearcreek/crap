...
from networks_gan import JSRCascade, Discriminator, MultiResDiscriminator,JSRCascade_v2
import losses
...
def train_reg_epoch(args, G, train_dataloader, opt_G, device, scaler=None,scheduler_lambda = None):
    ...
    for _, batch in tqdm(enumerate(train_dataloader), 'reg phase:'):
        ...
        if scaler is not None:
            with torch.cuda.amp.autocast():
                ...
                uncert = flows[4]
                # uncert = torch.clamp(flows[4],min=1e-20)
                alpha_x = uncert[0,0,:,:,:]
                beta_x = uncert[0,1,:,:,:]
                mu_x = uncert[0,2,:,:,:]
                ...
                sim_mind = losses.MIND_metric((128,160,128),d=1, patch_size=7, use_gaussian_kernel=True).loss(mrs_warped[0],ct_fixed_synths[0].detach())
                sim_ncc = 1 + losses.NCC(device='cuda').loss(synths_warped[0], ct_fixed_synths[0].detach())
                if args['lambda_multi']>0:
                    sim = args['lambda_multi']*sim_mind+(1-args['lambda_multi'])*sim_ncc
                else:
                    sim = sim_ncc
                
                log1x = torch.log(math.pi/mu_x)
                log2x = torch.log(omega_x)
                log3x = torch.log((sim)**2*mu_x+omega_x)
                log4x = torch.lgamma(alpha_x)-torch.lgamma(alpha_x+0.5)

                
                unc_x = 0.5*log1x-alpha_x*log2x+(alpha_x+0.5)*log3x+log4x
                reg_x = (2*mu_x+alpha_x)*torch.abs((sim))
                ...
                loss_unc = loss_uncx.mean()
            # scaler.scale(loss_reg.mean()).backward()
            scaler.scale(loss_unc).backward()
            # scaler.scale(args['lambda_reg']*loss_reg+lambd*loss_unc).backward()
            scaler.step(opt_G)
            scaler.update()
            ...
            # (args['lambda_reg']*loss_reg+lambd*loss_unc).backward()
            loss_unc.backward()
            #loss_reg.backward()
        ...
        if total_loss != total_loss:
            print(total_loss / num_samples, sim_loss / num_samples, smooth_loss / num_samples, unc_loss/num_samples, seg_loss/num_samples,\
                  log1_xloss/num_samples,log2_xloss/num_samples,log3_xloss/num_samples,reg_xloss,reg_yloss,reg_zloss)
            break


    return total_loss / num_samples, sim_loss / num_samples, smooth_loss / num_samples, unc_loss/num_samples, seg_loss/num_samples,\
                  log1_xloss/num_samples,log2_xloss/num_samples,log3_xloss/num_samples,reg_xloss/num_samples
...
def train_joint_epoch(args, G, D_MR, D_CBCT, train_dataloader, opt_G, opt_D_MR, opt_D_CBCT, device, scaler=None):
    ...
    for _, batch in tqdm(enumerate(train_dataloader), 'joint phase:'):
        if args['weak_supervision']:
            ...
            cbct_fixed, mr_moving, ct_fixed, ct_moving,flow = batch
        batch_size = cbct_fixed.size(0)
        num_samples += batch_size
        cbct_fixed = cbct_fixed.float().unsqueeze(1).to(device)
        mr_moving = mr_moving.float().unsqueeze(1).to(device)
        ...
        if scaler is not None:
            with torch.cuda.amp.autocast():
                ...
                loss_reg = (loss_sim + loss_smooth + loss_seg) / 4
                # mr_syn = (synths_warped[0]-synths_warped[0].min())/(synths_warped[0].max()-synths_warped[0].min())
                # cbct_syn = (ct_fixed_synths[0]-ct_fixed_synths[0].min())/(ct_fixed_synths[0].max()-ct_fixed_synths[0].min())
                # sim = torch.nn.AvgPool3d(kernel_size=3, stride = 1, padding =1 )(mr_syn*cbct_syn)
                sim_mind = losses.MIND_metric((128,160,128),d=1, patch_size=7, use_gaussian_kernel=True).loss(mrs_warped[0],ct_fixed_synths[0].detach())
                sim_ncc = 1 + losses.NCC(device='cuda').loss(synths_warped[0], ct_fixed_synths[0].detach())
                if args['lambda_multi']>0:
                    sim = args['lambda_multi']*sim_mind+(1-args['lambda_multi'])*sim_ncc
                    ...
                    sim = sim_ncc
                loss_unc = (0.5*torch.log(math.pi/mu_x)-alpha_x*torch.log(omega_x)+(alpha_x+0.5)*torch.log((sim).detach()**2*mu_x+omega_x)+torch.lgamma(alpha_x)-torch.lgamma(alpha_x+0.5)\
                                +args['lambda_inunc']*(2*mu_x+alpha_x)*abs((sim).detach())).mean()

                # Total Loss
                loss_G = loss_synth + args['lambda_reg'] * loss_reg + args['lambda_unc'] * loss_unc
            ...
            # cbct_syn = (ct_fixed_synths[0]-ct_fixed_synths[0].min())/(ct_fixed_synths[0].max()-ct_fixed_synths[0].min())
            # sim = torch.nn.AvgPool3d(kernel_size=3, stride = 1, padding =1 )(mr_syn*cbct_syn)
            # sim = losses.MIND_metric((128,160,128),d=1, patch_size=7, use_gaussian_kernel=True).loss(mrs_warped[0],ct_fixed_synths[0].detach())
            sim_mind = losses.MIND_metric((128,160,128),d=1, patch_size=7, use_gaussian_kernel=True).loss(mrs_warped[0],ct_fixed_synths[0].detach())
            sim_ncc = 1 + losses.NCC(device='cuda').loss(synths_warped[0], ct_fixed_synths[0].detach())
            if args['lambda_multi']>0:
                sim = args['lambda_multi']*sim_mind+(1-args['lambda_multi'])*sim_ncc
    ...
    return total_loss / num_samples, synth_loss / num_samples, reg_loss / num_samples, \
           sim_loss / num_samples, smooth_loss / num_samples, disc_loss / num_samples, unc_loss / num_samples 
...
def save_model(G, D_MR, D_CBCT, opt_G, opt_D_MR, opt_D_CBCT, args, epoch):
    torch.save({
        ...
        'args': args
    }, os.path.join(args['save_path'], 'JSREDLuns_exp3_nosl_mix_{}_ep{}.pt'.format(args['exp_name'], epoch)))
...
def get_config(train_path, test_path, exp_name, save_path='./checkpoint/', pretrained=None, amp=False,
               ...
               synthesis_checkpoint_mod=10, registration_checkpoint_mod=10, jsr_checkpoint_mod=10, lambda_inunc = 1, lambda_unc = 1):
    args = dict()
...
if __name__ == '__main__':
    ...
    scheduler = optim.lr_scheduler.StepLR(opt_G, step_size=20, gamma=0.1)
    # scheduler_warmup = GradualWarmupScheduler(opt_G, multiplier=1, total_epoch=5, after_scheduler=scheduler)

    # Optimizer Scheduler
    scheduler = optim.lr_scheduler.StepLR(opt_G, step_size=30, gamma=0.2)
    # scheduler_warmup = GradualWarmupScheduler(opt_G, multiplier=1, total_epoch=5, after_scheduler=scheduler)

    # Prepare Tensorboard
    boardio = SummaryWriter(log_dir='./log/JSRCascade_mix_exp3_nosl_{}_{}_{}_{}'.format(args['exp_name'], args['lambda_l1'],
    ...
    for i in range(args['schedule'][1]):
        lambd = math.exp(0.115*i-13.8)
        reg_loss, sim_loss, smoothness_loss,loss_unc, seg_loss, log1_loss, log2_loss,log3_loss,regularization_loss = train_reg_epoch(args, G, train_dataloader, opt_G, device, scaler, scheduler_lambda= None)
        ...
        boardio.add_scalar('unc_loss',loss_unc,epoch)
        boardio.add_scalar('seg_loss', seg_loss, epoch)
        boardio.add_scalar('log1_loss',log1_loss, epoch)
        boardio.add_scalar('log2_loss', log2_loss, epoch)
        boardio.add_scalar('log3_loss', log3_loss, epoch)
        boardio.add_scalar('regularization_loss', regularization_loss, epoch)
    ...
    for i in range(args['schedule'][2]):
        ...
        smoothness_loss, disc_loss, unc_loss = train_joint_epoch(args, G, D_MR, D_CBCT, train_dataloader,
                                                       opt_G, opt_D_MR, opt_D_CBCT, device, scaler)
        valid_l1_loss, valid_ncc_loss, valid_seg_loss = valid_epoch(G, valid_dataset, device)
        scheduler.step()
        print('joint epoch: {}, total_loss: {}, synthesis_loss: {}, reg_loss: {}, sim_loss: {}, smoothness_loss: {}, '
              'disc_loss: {}, unc_loss: {}'.format(epoch, total_loss, synth_loss, reg_loss, sim_loss, smoothness_loss, disc_loss, unc_loss))
        boardio.add_scalar('total_loss', total_loss, epoch)
