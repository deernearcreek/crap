...
def train_reg_epoch(args, G, train_dataloader, opt_G, device, scaler=None):
    checkpoint = torch.load('JSRCascade_newflow_l1100_multi0.3_reg100_seg6_ep99.pt',map_location=device)
    ...
    for _, batch in tqdm(enumerate(train_dataloader), 'reg phase:'):
        ...
        if scaler is not None:
            with torch.cuda.amp.autocast():
                ...
                uncert = flows[-1]
                loss_unc = (0.5*torch.exp(-uncert)*(flows[0]-gt)**2+0.5*uncert).mean()
    ...
    for _, batch in tqdm(enumerate(train_dataloader), 'joint phase:'):
        ...
        if scaler is not None:
            with torch.cuda.amp.autocast():
                ...
                loss_G = loss_synth + args['lambda_reg'] * loss_unc
...
def save_model(G, D_MR, D_CBCT, opt_G, opt_D_MR, opt_D_CBCT, args, epoch):
    torch.save({
        ...
        'args': args
    }, os.path.join(args['save_path'], 'JSRBNN_all_md4_sup_{}_ep{}.pt'.format(args['exp_name'], epoch)))
...
def get_config(train_path, test_path, exp_name, save_path='./checkpoint/', pretrained=None, amp=False,
               ...
               synthesis_checkpoint_mod=10, registration_checkpoint_mod=10, jsr_checkpoint_mod=10,lambda_unc = 0.1, lambda_inunc=2):
    args = dict()
