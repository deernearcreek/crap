    ...
    for _, batch in tqdm(enumerate(train_dataloader), 'reg phase:'):
        ...
        if scaler is not None:
            with torch.cuda.amp.autocast():
                ...
                synths_warped = [G.decoder_reg.STN4(ct_moving_synths[0].detach(), flows[0],affine0),
                                 G.decoder_reg.STN3(ct_moving_synths[1].detach(), flows[1],affine1),
                                 G.decoder_reg.STN2(ct_moving_synths[2].detach(), flows[2],affine2),
                                 G.decoder_reg.STN1(ct_moving_synths[3].detach(), flows[3],affine3)]
                if args['lambda_multi'] > 0:
                    mrs_warped = [G.decoder_reg.STN4(mr_moving, flows[0],affine0),
                                  G.decoder_reg.STN3(torch.nn.AvgPool3d(2)(mr_moving), flows[1],affine1),
                                  G.decoder_reg.STN2(torch.nn.AvgPool3d(4)(mr_moving), flows[2],affine2),
                                  G.decoder_reg.STN1(torch.nn.AvgPool3d(8)(mr_moving), flows[3],affine3)]
                if args['weak_supervision']:
                    segs_fixed = [seg_fixed, torch.nn.AvgPool3d(2)(seg_fixed),
                                  torch.nn.AvgPool3d(4)(seg_fixed),
                                  torch.nn.AvgPool3d(8)(seg_fixed)]
                    segs_warped = [G.decoder_reg.STN4(seg_moving, flows[0],affine0),
                                   G.decoder_reg.STN3(torch.nn.AvgPool3d(2)(seg_moving), flows[1],affine1),
                                   G.decoder_reg.STN2(torch.nn.AvgPool3d(4)(seg_moving), flows[2],affine2),
                                   G.decoder_reg.STN1(torch.nn.AvgPool3d(8)(seg_moving), flows[3],affine3)]
...
def valid_epoch(G, valid_dataloader, device):
    ...
    for i, batch in tqdm(enumerate(valid_dataloader), 'joint phase:'):
        ...
        with torch.no_grad():
            ...
            affine2 = flows[6]
            affine3 = flows[7]
            synth_warped = G.decoder_reg.STN4(ct_moving_synths[0].detach(), flows[0],affine0)
            seg_warped = G.decoder_reg.STN4(seg_moving, flows[0],affine0)
...
def save_model(G, D_MR, D_CBCT, opt_G, opt_D_MR, opt_D_CBCT, args, epoch):
    torch.save({
        ...
        'args': args
    }, os.path.join(args['save_path'], 'JSRCascade_{}_ep{}.pt'.format(args['exp_name'], epoch)))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--save_path", type=str, default="~rerun_runze/original_jsr/log",
