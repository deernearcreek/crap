    ...
    for _, batch in tqdm(enumerate(train_dataloader), 'joint phase:'):
        ...
        if scaler is not None:
            with torch.cuda.amp.autocast():
                ...
                sim = losses.MIND_metric((128,160,128),d=1, patch_size=7, use_gaussian_kernel=True).loss(mrs_warped[0],ct_fixed_synths[0])
...
def save_model(G, D_MR, D_CBCT, opt_G, opt_D_MR, opt_D_CBCT, args, epoch):
    torch.save({
        ...
        'args': args
    }, os.path.join(args['save_path'], 'JSREDLuns_all_mind_{}_ep{}.pt'.format(args['exp_name'], epoch)))
